...
class SparK(nn.Module):
        ...
        for i in range(
            ...
            if i == 0 and e_width == d_width:
                ...
                densify_proj = nn.Conv2d(
                    e_width, d_width, kernel_size=kernel_size, stride=1, padding=kernel_size // 2, bias=True
                )
                print(
    ...
    def forward(self, inp_bchw: torch.Tensor, active_b1ff=None, vis=False):
        ...
        active_b1hw = active_b1ff.repeat_interleave(self.downsample_raito, 2).repeat_interleave(
        ...
        if inp_bchw.shape[1] == 1:
            rec_bchw = rec_bchw[:, 0:1]
